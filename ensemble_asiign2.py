# -*- coding: utf-8 -*-
"""
Created on Thu Feb 25 03:59:56 2021
...
import pandas as pd
import numpy as np
cocoa_data=pd.read_excel(r"C:\Users\Amarnadh Tadi\Desktop\datascience\assign9\Coca_Rating_Ensemble.xlsx")
cocoa_data.columns
cocoa_data.dtypes
cocoa_data.drop(['Bean_Type','Origin'],axis='columns',inplace=True)
from sklearn.preprocessing import LabelEncoder
le=LabelEncoder()
cocoa_data['Company']=le.fit_transform(cocoa_data['Company'])
cocoa_data['Name']=le.fit_transform(cocoa_data['Name'])
cocoa_data['Company_Location']=le.fit_transform(cocoa_data['Company_Location'])
cocoa_data['Cocoa_Percent']=cocoa_data['Cocoa_Percent'].astype("int32")



cocoa_data['Cocoa_Percent'].unique
cocoa_data['Cocoa_Percent'].value_counts()
predictors=cocoa_data.drop(['Cocoa_Percent'], axis='columns')
predictors.head()
target=cocoa_data.iloc[:,[4]]
...
cocoa_data.columns
array=cocoa_data.values
x=array[:,0:6]
y=array[:,6]
train_x, train_y = x[:1300], y[:1300]
test_x, test_y = x[1300:], y[1300:]
